...
# Enable arm64 stuff
scripts/config -e AMPERE_ERRATUM_AC04_CPU_23
scripts/config -e ARM64_ERRATUM_2077057
scripts/config -e ARM64_ERRATUM_3194386
scripts/config -e ARM64_POE
scripts/config -e ARM_MHU
scripts/config -e ARM_QCOM_CPUFREQ_NVMEM
scripts/config -e ARM_SCPI_PROTOCOL
scripts/config -e ARM_SMC_WATCHDOG
scripts/config -e ARM_SP805_WATCHDOG
scripts/config -e BACKLIGHT_ODIN2MINI
scripts/config -e BACKLIGHT_SY7758
scripts/config -e CC_OPTIMIZE_FOR_PERFORMANCE
scripts/config -e COMMON_CLK_SCMI
scripts/config -e CPU_FREQ_DEFAULT_GOV_PERFORMANCE
scripts/config -e CRYPTO_AES_ARM64
scripts/config -e CRYPTO_DEV_QCE
scripts/config -e CRYPTO_USER_API_ENABLE_OBSOLETE
scripts/config -e DEBUG_INFO_DWARF_TOOLCHAIN_DEFAULT
scripts/config -e DEBUG_INFO_REDUCED
scripts/config -e DEBUG_MISC
scripts/config -e DEFAULT_SECURITY_DAC
scripts/config -e DMA_RESTRICTED_POOL
scripts/config -e DRM_LONTIUM_LT8912B
scripts/config -e DRM_MSM_DSI
scripts/config -e DRM_MSM_HDMI
scripts/config -e DRM_MSM_MDP4
scripts/config -e DRM_MSM_MDP5
scripts/config -e DRM_PANEL_AR02_3INCH
scripts/config -e DRM_PANEL_AR06_4INCH
scripts/config -e DRM_PANEL_AR11_5INCH
scripts/config -e DRM_PANEL_AYANEO_WT0600_2K
scripts/config -e DRM_PANEL_BOE_XM91080G
scripts/config -e DRM_PANEL_CHIPONE_ICNA35XX
scripts/config -e DRM_PANEL_DDIC_CH13726A
scripts/config -e DRM_PANEL_RETROID_POCKET_6
scripts/config -e DRM_PANEL_SYNAPTICS_TD4328
scripts/config -e FUNCTION_ALIGNMENT
scripts/config -e HISI_PMU
scripts/config -e HZ_250
scripts/config -e I2C_QUP
scripts/config -e I2C_RK3X
scripts/config -e INDIRECT_PIO
scripts/config -e INTERCONNECT_QCOM_SM8550
scripts/config -e IOMMU_DEFAULT_DMA_STRICT
scripts/config -e IOMMU_IO_PGTABLE_ARMV7S
scripts/config -e IOMMU_IO_PGTABLE_DART
scripts/config -e IP_PNP
scripts/config -e JOYSTICK_RSINPUT
scripts/config -e KVM
scripts/config -e LEDS_GROUP_MULTICOLOR
scripts/config -e LEDS_HTR3212
scripts/config -e LEGACY_PTYS
scripts/config -e LEGACY_TIOCSTI
scripts/config -e MHP_DEFAULT_ONLINE_TYPE_OFFLINE
scripts/config -e MMC_ARMMMCI
scripts/config -e MMC_DW
scripts/config -e MTD_CFI
scripts/config -e MTD_COMPLEX_MAPPINGS
scripts/config -e MTD_DATAFLASH
scripts/config -e MTD_NAND_DENALI_DT
scripts/config -e MTD_NAND_QCOM
scripts/config -e MTD_PHYSMAP
scripts/config -e MTD_QCOMSMEM_PARTS
scripts/config -e MTD_SST25L
scripts/config -e NFS_DISABLE_UDP_SUPPORT
scripts/config -e NO_HZ_IDLE
scripts/config -e NVMEM_QCOM_SEC_QFPROM
scripts/config -e OF_OVERLAY
scripts/config -e OPTEE
scripts/config -e OVERLAY_FS_REDIRECT_ALWAYS_FOLLOW
scripts/config -e PCIE_ALTERA
scripts/config -e PCIE_KIRIN
scripts/config -e PCI_ENDPOINT
scripts/config -e PCI_HISI
scripts/config -e PCI_HOST_THUNDER_ECAM
scripts/config -e PCI_HOST_THUNDER_PEM
scripts/config -e PCI_PWRCTRL_GENERIC
scripts/config -e PINCTRL_SM8550
scripts/config -e PLATFORM_MHU
scripts/config -e POWER_RESET_MSM
scripts/config -e PWM_SN3112
scripts/config -e QCOM_A53PLL
scripts/config -e QCOM_APCS_IPC
scripts/config -e QCOM_CPR
scripts/config -e QCOM_EBI2
scripts/config -e QCOM_HFPLL
scripts/config -e QCOM_IRQ_COMBINER
scripts/config -e QCOM_L2_PMU
scripts/config -e QCOM_L3_PMU
scripts/config -e QCOM_MPM
scripts/config -e QCOM_OCMEM
scripts/config -e QCOM_PBS
scripts/config -e QCOM_RMTFS_MEM
scripts/config -e QCOM_SMD_RPM
scripts/config -e QCOM_SMSM
scripts/config -e QCOM_SPM
scripts/config -e QCOM_SPMI_VADC
scripts/config -e RAID6_PQ_BENCHMARK
scripts/config -e RCU_TRACE
scripts/config -e REGULATOR_ARM_SCMI
scripts/config -e REGULATOR_QCOM_REFGEN
scripts/config -e REGULATOR_QCOM_SPMI
scripts/config -e REGULATOR_QCOM_USB_VBUS
scripts/config -e REGULATOR_SGM3804
scripts/config -e RMI4_F54
scripts/config -e ROCKCHIP_ERRATUM_3568002
scripts/config -e RPMSG_QCOM_SMD
scripts/config -e RTC_DRV_EFI
scripts/config -e SENSORS_ARM_SCMI
scripts/config -e SERIAL_8250_16550A_VARIANTS
scripts/config -e SERIAL_MSM
scripts/config -e SERIAL_OF_PLATFORM
scripts/config -e SERIAL_XILINX_PS_UART
scripts/config -e SM_CAMCC_8550
scripts/config -e SM_GCC_8550
scripts/config -e SM_GPUCC_8550
scripts/config -e SM_TCSRCC_8550
scripts/config -e SM_VIDEOCC_8550
scripts/config -e SND_SOC_SOF_OF
scripts/config -e SND_SUPPORT_OLD_API
scripts/config -e SPI_QCOM_QSPI
scripts/config -e SPI_QUP
scripts/config -e SPMI_PMIC_CLKDIV
scripts/config -e SRAM
scripts/config -e SYSCON_REBOOT_MODE
scripts/config -e SYSFS_SYSCALL
scripts/config -e TOUCHSCREEN_HYNITRON_ALL
scripts/config -e TRANSPARENT_HUGEPAGE_MADVISE
scripts/config -e TRANSPARENT_HUGEPAGE_SHMEM_HUGE_NEVER
scripts/config -e TRANSPARENT_HUGEPAGE_TMPFS_HUGE_NEVER
scripts/config -e USB_DWC3_OF_SIMPLE
scripts/config -e USB_ONBOARD_DEV
scripts/config -e USB_OTG
scripts/config -e USB_QCOM_EUD
scripts/config -e USB_SNP_UDC_PLAT
scripts/config -e USB_ULPI
scripts/config -e VEXPRESS_CONFIG
scripts/config -e ZRAM_DEF_COMP_LZORLE
scripts/config -m ARM_CCI_PMU
scripts/config -m ARM_CCN
scripts/config -m ARM_CMN
scripts/config -m ARM_CORESIGHT_PMU_ARCH_SYSTEM_PMU
scripts/config -m ARM_DMC620_PMU
scripts/config -m ARM_DSU_PMU
scripts/config -m ARM_SMMU_V3_PMU
scripts/config -m ARM_SPE_PMU
...
scripts/config -m DRM_SIMPLE_BRIDGE
scripts/config -m EEPROM_AT25
scripts/config -m INPUT_PM8XXX_VIBRATOR
scripts/config -m MTD_HYPERBUS
scripts/config -m MTD_NAND_BRCMNAND
scripts/config -m MUX_MMIO
scripts/config -m NVMEM_LAYOUT_SL28_VPD
scripts/config -m NVMEM_REBOOT_MODE
scripts/config -m PHY_CADENCE_DPHY_RX
scripts/config -m PHY_CADENCE_SALVO
scripts/config -m PHY_CADENCE_SIERRA
scripts/config -m PHY_CADENCE_TORRENT
scripts/config -m PHY_QCOM_M31_USB
scripts/config -m PHY_QCOM_PCIE2
scripts/config -m PHY_QCOM_QMP_PCIE_8996
scripts/config -m PHY_QCOM_QUSB2
scripts/config -m PHY_QCOM_SGMII_ETH
scripts/config -m PHY_QCOM_USB_HS_28NM
scripts/config -m PHY_QCOM_USB_SS
scripts/config -m QCOM_COINCELL
scripts/config -m QCOM_Q6V5_MSS
scripts/config -m QCOM_WCNSS_CTRL
scripts/config -m QCOM_WCNSS_PIL
scripts/config -m REGULATOR_VCTRL
scripts/config -m SLIM_QCOM_NGD_CTRL
scripts/config -m SND_AUDIO_GRAPH_CARD
scripts/config -m SND_AUDIO_GRAPH_CARD2
scripts/config -m SND_SOC_APQ8016_SBC
scripts/config -m SND_SOC_BT_SCO
scripts/config -m SND_SOC_FSL_ASRC
scripts/config -m SND_SOC_FSL_AUDMIX
scripts/config -m SND_SOC_FSL_ESAI
scripts/config -m SND_SOC_FSL_MICFIL
scripts/config -m SND_SOC_FSL_SAI
scripts/config -m SND_SOC_FSL_SPDIF
scripts/config -m SND_SOC_FSL_SSI
scripts/config -m SND_SOC_IMX_AUDMUX
scripts/config -m VIDEO_MAX96712
scripts/config -m VIDEO_QCOM_IRIS
scripts/config -e ARCH_HAS_PKEYS
scripts/config -e ARM64_LPA2
scripts/config -e ARM64_PA_BITS_52
scripts/config -e ARM_CCI
scripts/config -e ARM_CCI400_COMMON
scripts/config -e ARM_CCI400_PMU
scripts/config -e ARM_CCI5xx_PMU
scripts/config -e ARM_SCMI_HAVE_MSG
scripts/config -e ARM_SCMI_TRANSPORT_OPTEE
scripts/config -e ARM_SCPI_CPUFREQ
scripts/config -e ARM_SCPI_POWER_DOMAIN
scripts/config -e CLK_VEXPRESS_OSC
scripts/config -e COMMON_CLK_SCPI
scripts/config -e CRYPTO_DEV_QCE_AEAD
scripts/config -e CRYPTO_DEV_QCE_ENABLE_ALL
scripts/config -e CRYPTO_DEV_QCE_SHA
scripts/config -e CRYPTO_DEV_QCE_SKCIPHER
scripts/config -e DEFAULT_BBR
scripts/config -e DRM_MSM_DSI_10NM_PHY
scripts/config -e DRM_MSM_DSI_14NM_PHY
scripts/config -e DRM_MSM_DSI_20NM_PHY
scripts/config -e DRM_MSM_DSI_28NM_8960_PHY
scripts/config -e DRM_MSM_DSI_28NM_PHY
scripts/config -e DRM_MSM_DSI_7NM_PHY
scripts/config -e DRM_MSM_HDMI_HDCP
scripts/config -e GUEST_PERF_EVENTS
scripts/config -e HAVE_KERNEL_GZIP
scripts/config -e HAVE_KERNEL_ZSTD
scripts/config -e HAVE_KVM_CPU_RELAX_INTERCEPT
scripts/config -e HAVE_KVM_DIRTY_RING
scripts/config -e HAVE_KVM_DIRTY_RING_ACQ_REL
scripts/config -e HAVE_KVM_IRQCHIP
scripts/config -e HAVE_KVM_IRQ_BYPASS
scripts/config -e HAVE_KVM_IRQ_ROUTING
scripts/config -e HAVE_KVM_MSI
scripts/config -e HAVE_KVM_READONLY_MEM
scripts/config -e HAVE_KVM_VCPU_RUN_PID_CHANGE
scripts/config -e HW_RANDOM_OPTEE
scripts/config -e IP_PNP_BOOTP
scripts/config -e IP_PNP_DHCP
scripts/config -e KVM_COMMON
scripts/config -e KVM_GENERIC_DIRTYLOG_READ_PROTECT
scripts/config -e KVM_GENERIC_HARDWARE_ENABLING
scripts/config -e KVM_GENERIC_MMU_NOTIFIER
scripts/config -e KVM_GUEST_MEMFD
scripts/config -e KVM_MMIO
scripts/config -e KVM_VFIO
scripts/config -e LOGO_LINUX_CLUT224
scripts/config -e MFD_VEXPRESS_SYSREG
scripts/config -e MMC_DW_PLTFM
scripts/config -e MMC_QCOM_DML
scripts/config -e MTD_CFI_ADV_OPTIONS
scripts/config -e MTD_CFI_AMDSTD
scripts/config -e MTD_CFI_INTELEXT
...
scripts/config -e NEED_KVM_DIRTY_RING_WITH_BITMAP
scripts/config -e OF_RESOLVE
scripts/config -e PCIE_ALTERA_MSI
scripts/config -e PCI_ENDPOINT_CONFIGFS
scripts/config -e PREEMPT_NOTIFIERS
scripts/config -e QCOM_CLK_SMD_RPM
scripts/config -e QCOM_KRYO_L2_ACCESSORS
scripts/config -e QCOM_RPMPD
scripts/config -e REGULATOR_QCOM_SMD_RPM
scripts/config -e SENSORS_ARM_SCPI
scripts/config -e SERIAL_FSL_LINFLEXUART_CONSOLE
scripts/config -e SERIAL_FSL_LPUART_CONSOLE
scripts/config -e SERIAL_MSM_CONSOLE
scripts/config -e SERIAL_XILINX_PS_UART_CONSOLE
scripts/config -e SM_DISPCC_8550
scripts/config -e SND_COMPRESS_ACCEL
scripts/config -e SND_SOC_SOF_MTK_TOPLEVEL
scripts/config -e SQUASHFS_COMPILE_DECOMP_SINGLE
scripts/config -e TICK_CPU_ACCOUNTING
scripts/config -e USB_ULPI_VIEWPORT
scripts/config -m CRYPTO_ARC4
scripts/config -m CRYPTO_DEV_HISI_QM
scripts/config -m MTD_NAND_BRCMNAND_BCMBCA
scripts/config -m MTD_NAND_BRCMNAND_BRCMSTB
scripts/config -m MTD_NAND_BRCMNAND_IPROC
scripts/config -m NVIDIA_CORESIGHT_PMU_ARCH_SYSTEM_PMU
scripts/config -m PCI_EPF_TEST
scripts/config -m SND_SOC_FSL_EASRC
scripts/config -m SND_SOC_LPASS_APQ8016
scripts/config -m SND_SOC_LPASS_CPU
scripts/config -m SND_SOC_LPASS_PLATFORM
#scripts/config --set-val BLK_DEV_LOOP_MIN_COUNT 8
#scripts/config --set-val CMA_AREAS 20
#scripts/config --set-val CMA_SIZE_MBYTES 32
#scripts/config --set-val COMPACT_UNEVICTABLE_DEFAULT 1
#scripts/config --set-val FUNCTION_ALIGNMENT 4
#scripts/config --set-val HZ 250
#scripts/config --set-val IP_VS_TAB_BITS 12
#scripts/config --set-val LD_VERSION 24400
#scripts/config --set-val MAGIC_SYSRQ_DEFAULT_ENABLE 0x1
#scripts/config --set-val MMC_BLOCK_MINORS 32
#scripts/config --set-val NODES_SHIFT 4
#scripts/config --set-val NR_CPUS 512
#scripts/config --set-val PAHOLE_VERSION 0
#scripts/config --set-val PGTABLE_LEVELS 5
#scripts/config --set-val RCU_CPU_STALL_TIMEOUT 21
#scripts/config --set-val SERIAL_8250_NR_UARTS 8
#scripts/config --set-val SERIAL_8250_RUNTIME_UARTS 4
#scripts/config --set-val THERMAL_EMERGENCY_POWEROFF_DELAY_MS 0
#scripts/config --set-val VGA_ARB_MAX_GPUS 16
#scripts/config --set-str ZRAM_DEF_COMP "lzo-rle"
#scripts/config --set-val CRYPTO_DEV_QCE_SW_MAX_LEN 512
